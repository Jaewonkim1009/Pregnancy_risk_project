...
# 사이드바에서 데이터 입력받기
age = st.sidebar.number_input("나이", min_value=20, max_value=70, value=30)
...
body_temp_c = st.sidebar.number_input("체온 (°C)", min_value=35.0, max_value=41.0, value=36.5, step=0.1)
body_temp = (body_temp_c * 9/5) + 32  # 섭씨 -> 화씨 변환
heart_rate = st.sidebar.number_input("심박수 (bpm)", min_value=60, max_value=100, value=75)
height = st.sidebar.number_input("키 (cm)", min_value=140.0, max_value=190.0, value=165.0, step=0.1)
weight = st.sidebar.number_input("체중 (kg)", min_value=30.0, max_value=100.0, value=60.0, step=0.1)

bmi = weight / ((height/100) ** 2)
gestational_age = st.sidebar.number_input("임신 주수", min_value=10, max_value=40, value=28)
